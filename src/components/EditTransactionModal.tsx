...
export function EditTransactionModal({ open, onOpenChange, transaction }: any) {
  const { editTransaction, entities, categoriesByEntity } = useFinance()
  ...
  useEffect(() => {
    if (open && transaction) {
      form.reset({
        ...
        date: new Date(transaction.rawDate + 'T00:00:00'),
        type: transaction.originalType || 'out',
